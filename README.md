...
1. Clang - фронтенд компилятора для языков C/C++/Objective-C, преобразует исходный код в LLVM IR.

2. LLVM - инфраструктура компиляции, использующая промежуточное представление (IR) и набор оптимизаций, применяемых независимо от исходного языка.

3. AST - древовидная структура, сохраняющая синтаксис исходного кода. LLVM IR - трёхадресный код в SSA-форме, близкий к машинному.

4. IR позволяет:
    - Применять оптимизации независимо от языка
    - Упрощать анализ кода
    - Обеспечивать многопроходность компиляции

5. alloca выделяет память на стеке для переменной. Используется до оптимизаций для размещения переменных в памяти.

6. Оптимизации нужны для:
    - Увеличения скорости выполнения
    - Уменьшения размера кода
    - Экономии энергии

7. SSA-форма (Static Single Assignment) - каждая переменная присваивается один раз, что упрощает анализ потока данных и оптимизации.

8. CFG показывает все возможные пути выполнения функции, помогает анализировать недостижимый код и оптимизировать ветвления.

9. Арифметические операции в LLVM IR - это инструкции add, sub, mul, div, работающие с SSA-значениями.

10. Функции - единицы анализа и оптимизации, так как имеют чёткие границы, локальные переменные и независимый CFG.
Короткие функции, вызываемые один раз, обычно встраиваются (inlining) оптимизатором для устранения накладных расходов на вызов.

11. IR и CFG дают преимущества:
    - Языконезависимость
    - Упрощённая SSA-форма
    - Возможность многопроходных оптимизаций
    - Чёткое разделение между анализом и трансформацией
